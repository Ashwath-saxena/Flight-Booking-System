...
export default function RoundTripBookingConfirmationPage() {
  ...
  const outboundBookingId = searchParams.get("outboundBookingId") as string;
  const returnBookingId = searchParams.get("returnBookingId") as string;

  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const [outbound, setOutbound] = useState<BookingDetails | null>(null);
  const [outboundPassengerSeats, setOutboundPassengerSeats] = useState<PassengerWithSeat[]>([]);
  const [returnFlight, setReturnFlight] = useState<BookingDetails | null>(null);
  const [returnPassengerSeats, setReturnPassengerSeats] = useState<PassengerWithSeat[]>([]);

  const [cancelling, setCancelling] = useState(false);
  const [flightIdCopied, setFlightIdCopied] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    async function fetchBooking(bookingId: string, setter: (b: BookingDetails | null) => void) {
      if (!bookingId) return;
      try {
    ...
    setLoading(true);
    Promise.all([
      fetchBooking(outboundBookingId, setOutbound),
      fetchBooking(returnBookingId, setReturnFlight),
    ]).finally(() => setLoading(false));
  }, [user, outboundBookingId, returnBookingId, router]);

  useEffect(() => {
    if (!user) return;
    async function fetchPassengerSeats(bookingId: string, setter: (p: PassengerWithSeat[]) => void) {
      if (!bookingId) return;
      ...
      if (seatsError) {
        setter(passengers as PassengerWithSeat[]);
        return;
      }
      ...
      setter(passengersWithSeats);
    }
    fetchPassengerSeats(outboundBookingId, setOutboundPassengerSeats);
    fetchPassengerSeats(returnBookingId, setReturnPassengerSeats);
  }, [outboundBookingId, returnBookingId, user]);

  const copyFlightId = (flightId: string) => {
    navigator.clipboard.writeText(flightId);
    setFlightIdCopied(flightId);
    toast.success("Flight Tracking ID copied!");
    setTimeout(() => setFlightIdCopied(null), 3000);
  };
  ...
  const handleCancelRoundTrip = async () => {
    if (
      ...
      !user ||
      !confirm(
        "Are you sure you want to cancel this round-trip booking? This action cannot be undone."
      )
    ) {
      return;
    }
    setCancelling(true);
    try {
      // Cancel both bookings: outbound and return
      for (const booking of [outbound, returnFlight]) {
        ...
        // Send cancellation email for each segment
      ...
      toast.success("Round-trip booking cancelled successfully");
    } catch (err) {
      console.error("Error cancelling round-trip booking:", err);
  ...
  const generateETicket = (booking: BookingDetails, passengers: PassengerWithSeat[]) => {
    if (!booking) return;
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {(outbound.booking_status === "Confirmed" || returnFlight.booking_status === "Confirmed") && (
          <motion.div
          ...
          >
            ...
            <span className="text-green-800 font-medium">
              Round-trip booking confirmed successfully!
            </span>
          </motion.div>
        )}

        {(outbound.booking_status === "Cancelled" && returnFlight.booking_status === "Cancelled") && (
          <motion.div
          ...
          >
            ...
            <span className="text-red-800 font-medium">
              This round-trip booking has been cancelled
            </span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {[{ label: "Outbound Flight", booking: outbound, passengerSeats: outboundPassengerSeats },
        { label: "Return Flight", booking: returnFlight, passengerSeats: returnPassengerSeats }]
        .map(({ label, booking, passengerSeats }, idx) => (
        <motion.div
          key={label}
        ...
        >
          <div className="p-6 border-b bg-blue-50">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center">
              <div>
                <div className="text-gray-700">Booking Reference</div>
                <div className="text-gray-900 font-semibold">{booking.id}</div>
              </div>
              <div>
                <span
          ...
          <div className="p-6 bg-blue-50 flex flex-wrap gap-2 items-center">
            <h3 className="font-medium text-blue-900 mb-2 mr-2 flex items-center">
            ...
            <button
              onClick={() => copyFlightId(booking.flight.id)}
              className={`px-3 py-2 rounded text-sm font-medium transition-colors ${
                flightIdCopied === booking.flight.id
                  ? 'bg-green-600 text-white'
                  : 'bg-blue-600 hover:bg-blue-700 text-white'
              }`}
            >
              {flightIdCopied === booking.flight.id ? '✓ Copied!' : 'Copy ID'}
            </button>
            <Link
            ...
            <button
              onClick={() => generateETicket(booking, passengerSeats)}
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between">
        <motion.button
          onClick={handleCancelRoundTrip}
          disabled={cancelling || (outbound.booking_status === "Cancelled" && returnFlight.booking_status === "Cancelled")}
          ...
          whileTap={{ scale: 0.97 }}
        >
          {cancelling ? "Cancelling..." : "Cancel Round-trip Booking"}
        </motion.button>
        <Link
          href="/bookings"
